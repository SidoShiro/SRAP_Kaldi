...
cat $dir/lexicon_words.txt | awk '{ for(n=2;n<=NF;n++){ phones[$n] = 1; }} END{for (p in phones) print p;}' | \
  egrep -v 'SIL|\+b\+|\+par\+|\+i\+|\+mus\+' | sort > $dir/nonsilence_phones.txt

( echo SIL; echo NSN; echo '+b+'; echo '+par+'; echo '+i+'; echo '+mus+' ) > $dir/silence_phones.txt
