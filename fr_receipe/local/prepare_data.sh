...
;;'
    # Process the STMs
    cat db/DECODA/$set/stm/*.stm | sort -k1,1 -k2,2 -k4,4n | \
  ...
  cat $dir/spk2utt | awk -v set=$set -v pwd=$PWD '{ printf("%s %s/db/DECODA/%s/wav/%s.wav \n", $1, pwd, set, $1); }' > $dir/wav.scp
  ...
  # especially at the beginning of segments.  Extend the times.
    ...
    # utiils/data/extend_segment_times.py --start-padding=0.15 \
