...
if [ $stage -le 0 ]; then
  ...
  # Unzip DECODA 6 data sources, normalize apostrophe+suffix to previous word, gzip the result.
  #gunzip -c db/DECODA/LM/*.en.gz | sed 's/ <\/s>//g' | local/join_suffix.py | gzip -c  > ${dir}/data/text/train.txt.gz
  ...
  # get wordlist
  awk '{print $1}' db/DECODA/decoda.dict | sed 's:([0-9])::g' | sort | uniq > ${dir}/data/wordlist
fi
...
if [ $stage -le 3 ]; then
  ...
  get_data_prob.py ${dir}/data/real_dev_set.txt ${dir}/data/lm_${order}_prune_small
